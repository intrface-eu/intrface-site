...
- **About, four moves.** The same ground and the same panes as home, in four: the claim, set like the home hero with the field under it; the person — a large portrait plate that crosses the pane edge at `lg`, the name, and a short first-person paragraph beside it; one `data-ground-key="land"` band where the map gathers and marks Vrsar, with no phrases, caption or label, since "From here / to the world" stays a home moment; and a close of one sentence and the two home-hero buttons. No studio blurb, no product list, no second contact form. With one band only, the field runs contour → land → contour and never reaches the mark.
...
- Ground, live form (`VectorGround`, WebGL2, motion allowed): a field of short `--ink` lines, one per vertex of an 18px grid, each turned to point at the pointer so the field reads as rays into the hand; the pointer settles rather than snaps, and drifts on its own after two seconds still or on a coarse pointer. Scroll moves the field through four states keyed to the two places the page leaves for it: one `.ground-band` viewport of open paper (`data-ground-key="land"`, between Selected work and What we build) and the bottom edge of the contact close (`data-ground-key="mark"`), the line the close joins the footer on, so the close keeps the contour map and the mark gathers only as the close leaves the screen, whole over the footer: under the hero a contour map, each line lying along the isoline of a height map that scroll shifts, strong on a contour level and faint between, with a hill under the hand the contours ring → the outline of Istria as a plate → the same contour map between the bands → the mark as a hatched plate in three dimensions. One field style with two gatherings; the shader's current (streamline) kind stays available but is off the sequence. About a third of the lines never join a plate; they stay in the field behind it so a gathering keeps its depth. Lines never sit at full strength behind type; the plates tilt with the pointer. Istria's outline is traced from the public-domain Natural Earth coastline and needs no credit. The land plate carries the site's one mark: a red X on Vrsar, snapped to the smoothed outer contour at the plate's top face so it sits on the coast and not in the hatch, drawn by about four dozen lines taken from the pool that would otherwise join the plate and set out late, so the X lands after the coast has settled. Once there they hold their stroke — the pointer turns the plate, never these lines — and they are the only red on the site. The X arriving is also what starts the two words in the band: “From here” slides out from under the X to its left, then “to the world” is written left to right in the band's bottom right corner, both in display type, ink, and `aria-hidden`. They are CSS from one attribute the ground sets when the X has arrived, so under reduced motion or without WebGL2, where the ground never activates, neither the X nor the words exist. While it runs, the panes open: paper panes give up their slabs and the copy sits on the ground, which quiets its lines to about a tenth within a soft margin of every block marked `data-ground-quiet`; ink panes keep their slab beneath the ground and the field runs through them in paper-coloured lines. Content has weight in the two open fields: each marked block pulls the lines toward it and turns them along its edges, in the contour map; the mark and Istria plates are left alone. The hero opens too: its dot screen is gone and the field runs under the claim. When the hand is still for two seconds, or on a coarse pointer, each line turns at its own rate, a share of them in 45° steps, so the ground looks busy rather than waiting.
