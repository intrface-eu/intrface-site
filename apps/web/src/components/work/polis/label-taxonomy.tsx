    ...
    <dl className={`divide-y divide-rule border-y border-rule ${className}`}>
      {LABELS.map((label, index) => (
        <div className="grid gap-x-8 gap-y-2 py-5 sm:grid-cols-[13rem_1fr] sm:items-baseline" key={label}>
          <dt>
            <span className="inline-flex rounded-full border border-rule px-3 py-1 font-mono text-[0.8rem]">
              {label}
            </span>
          </dt>
          <dd className="text-sm leading-6 text-ink-muted">{glosses[index]}</dd>
        </div>
      ))}
    </dl>
  );
